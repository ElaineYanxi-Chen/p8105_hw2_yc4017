...
```{r load pols_month, message = FALSE}
...
Here we have to make sure the key variables take the same values. The months are all spelled as lower cases in the `unemployment` dataset, but the first letters are capitalized in the `pols_month` and `snp` datasets.
