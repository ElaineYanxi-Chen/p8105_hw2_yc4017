...
First we will merge `snp` into `pols`. We can merge by the variables
`year` and `month`. We will first need to makes sure the merging
variables are of the same types.
...
-   There are 9 variables for 817 observations in this dataset.

-   The dataset describes the time (`year` and `month`) and the
    government situation in the US, including the the number of
    governors, senators, representatives of the Republican and the
    Democratic Party, and the party that the president was in on the
    associated date.

-   The dataset spans 68 years between years 1947, 2015.

### `snp`

-   There are 4 variables for 787 observations in this dataset.

-   The dataset describes the time (`year`, `month`, and `day`) and the
    variable `close`, the closing values of the Standard & Poor’s stock
    market index on the associated date, which is often used as a
    representative measure of stock market as a whole.

-   The dataset spans 65 years between years 1950, 2015.

### `unemployment`

-   There are 3 variables for 816 observations in this dataset.

-   The dataset describes the time (`year` and `month`) and the variable
    `unemploy_perc`, which indicates the percentage of unemployment in
    the associated month and year.

-   The dataset spans 67 years between years 1948, 2015.

### The merged dataset: `final_merge`

-   There are 12 variables for 817 observations in this dataset.

-   The dataset describes the time (`year`, `month` and `day`), the
    government composition in the US (number of Democrats and
    Republicans as representatives, governors, and senates, and the
    party that the President belongs to), the measure of the stock
    market (`close`), and the percentage of unemployment in the US on
    the associated dates (`unemploy_perc`).
